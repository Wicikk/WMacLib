...
function WMacLib:Window(Settings)
	...
	function WindowFunctions:TabGroup()
		...
		function SectionFunctions:Tab(Settings)
			...
			local contentRowSizeConstraint = Instance.new("UISizeConstraint")
			contentRowSizeConstraint.Name = "ContentRowSizeConstraint"
			contentRowSizeConstraint.MaxSize = Vector2.new(860, math.huge)
			contentRowSizeConstraint.Parent = contentRow
			...
			function TabFunctions:Section(Settings)
				...
				function SectionFunctions:Button(Settings, Flag)
					...
					buttonInteract.MouseButton1Down:Connect(function()
						if locked then return end
						Tween(buttonInteract, TweenInfo.new(0.08, Enum.EasingStyle.Sine), {
							TextTransparency = 0
						}):Play()
						Tween(buttonImage, TweenInfo.new(0.08, Enum.EasingStyle.Sine), {
							ImageTransparency = 0
						}):Play()
					end)
			...
			local function SelectCurrentTab()
				...
				local newContent = tabs[tabSwitcher].tabContent
				newContent.Position = UDim2.fromOffset(7, 63)
				newContent.Parent = content
				Tween(newContent, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
					Position = UDim2.fromOffset(0, 63)
				}):Play()
				currentTabInstance = newContent
